...
export default function GuardEmergencyContacts() {
  const { emergencyContacts } = useApp();

  return (
    <GuardLayout title="Emergency Contacts" breadcrumb="Home / Emergency Contacts">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
          {emergencyContacts.map((contact, i) => (
            <div className="contact-card" key={i}>
              ...
              <a href={`tel:${contact.phone}`} className="btn btn-success btn-sm">Call</a>
            </div>
          ))}
        </div>
      </div>
